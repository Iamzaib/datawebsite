...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.emailList.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.email-lists.store") }}" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                    <label class="required" for="list_title">{{ trans('cruds.emailList.fields.list_title') }}</label>
                    <input class="form-control {{ $errors->has('list_title') ? 'is-invalid' : '' }}" type="text" name="list_title" id="list_title" value="{{ old('list_title', '') }}" required>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control ckeditor {{ $errors->has('list_details') ? 'is-invalid' : '' }}" name="list_details" id="list_details">{!! old('list_details') !!}</textarea>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('list_price') ? 'is-invalid' : '' }}" type="number" name="list_price" id="list_price" value="{{ old('list_price', '') }}" step="0.01" required>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('record_type') ? 'is-invalid' : '' }}" name="record_type" id="record_type" required>
                        ...
                        @foreach(App\Models\Email::RECORD_TYPE_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('record_type', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
